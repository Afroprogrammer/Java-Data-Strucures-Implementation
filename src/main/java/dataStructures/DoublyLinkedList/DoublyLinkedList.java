...
public class DoublyLinkedList {
    ...
    public void  removeObject(int data){
        Node currrentNode = head;   //setting the tracker node
        while ((currrentNode != null && currrentNode.data != data))  //while loop to track
        {
            currrentNode = currrentNode.next;   //move to the next node
        }

        if (currrentNode == null) {   //removing from the head element
            System.out.println("current data not found");
            return;
        }
            Node previousNode= currrentNode.prev;   //make the next and previous node next point to the nextNode
            Node nextNode = currrentNode.next;  //make the next node's previous point to the previous node
        if (currrentNode == head) {   //removing from the head element
            head = head.next;   //head element would point to the next element
            head.prev = null;     //previous would be set to null
        } else if (currrentNode == tail) {
            tail = tail.next;
            tail.prev = null;
        } else {
            previousNode.next = nextNode;  //make the next and previous node next point to the nextNode
